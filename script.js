...
function criptografarMensagem(mensagem) {
    ...
    for (let i = 0; i < mensagem.length; i++) {
        switch (mensagem[i]) {
            case vogais[0]:
                palavraCriptogafada += criptografar[0];
                break;
            case vogais[1]:
                palavraCriptogafada += criptografar[1];
                break;
            case vogais[2]:
                palavraCriptogafada += criptografar[2];
                break;
            case vogais[3]:
                palavraCriptogafada += criptografar[3];
                break;
            case vogais[4]:
                palavraCriptogafada += criptografar[4];
                break;
            default:
                palavraCriptogafada += mensagem[i];
        }
    }
